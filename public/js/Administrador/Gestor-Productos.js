...
(function ($) {
    $(function () {
        ...
        window.mostrarModalEliminarProducto = function (id, nombre) {
            ...
            $("#formEliminarProducto").attr(
                "action",
                `/admin/productos/${id}?tab=${
        ...
        window.mostrarSugerenciasProducto = function (valor) {
            ...
            const listaId =
                activeId === "buscarProductosPendientes"
        ...
        $(document).on("click", function (e) {
            if (!$(e.target).closest(".input-group").length) {
                $(
                    "#sugerenciasProductoAprobados, #sugerenciasProductoPendientes"
                ).hide();
            }
        });
    });
...
// Ocultar alerta de éxito luego de 5 segundos (5000ms)
document.addEventListener("DOMContentLoaded", function () {
    const alerta = document.getElementById("alerta-exito");
    if (alerta) {
        setTimeout(() => {
            alerta.classList.remove("show"); // para cerrar automáticamente
            alerta.classList.add("fade"); // animación de desvanecimiento
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (alerta) {
        setTimeout(() => {
            ...
            setTimeout(() => alerta.remove(), 300); // Remueve del DOM
        }, 5000);
    }
});
