...
!function($){
  $.fn.parallax = function(){
    ...
    return this.each(function(){
      var $el = $(this)
        .prepend('<div class="material-parallax parallax"><img src="{{ asset("images/_blank.png") }}" alt=""></div>')
        .find('.material-parallax');

      function update(first){
        var h = ww < 601
          ? ($el.height() > 0 ? $el.height() : $el.children('img').height())
          : ($el.height() > 0 ? $el.height() : 500);

        var $img = $el.children('img').first(),
            imgH = $img.height(),
            offsetBottom = $el.offset().top + h,
            offsetTop = $el.offset().top,
            scrollY = $(window).scrollTop(),
            viewportH = window.innerHeight,
            pct = (scrollY + viewportH - offsetTop) / (h + viewportH),
            translateY = Math.round((imgH - h) * pct);

        if (first) $img.css('display','block');
        if (offsetBottom > scrollY && offsetTop < scrollY + viewportH) {
          $img.css('transform','translate3D(-50%,' + translateY + 'px, 0)');
        }
      }

      // Carga dinámica de la imagen real
      $el.children('img').first()
        .attr('src', $el.parents('[data-parallax-img]').data('parallax-img'))
        .one('load', function(){ update(true); })
        .each(function(){ if (this.complete) $(this).trigger('load'); });

      // Recalculamos al hacer scroll y resize
